...
export function renderEmail(opts: EmailTemplateOptions) {
  ...
  const title = opts.title ? `<h2 style="margin:0 0 8px 0;color:${primary};font-family:Arial,Helvetica,sans-serif;">${escapeHtml(opts.title)}</h2>` : ''
  const intro = opts.intro ? `<p style="margin:8px 0 0 0;color:${text}">${escapeHtml(opts.intro)}</p>` : ''
  const body = opts.body ? `<p style="margin:8px 0 0 0;color:${text}">${escapeHtml(opts.body)}</p>` : ''
