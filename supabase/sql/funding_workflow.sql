...
-- Ledger básico de movimientos (cobros prestatario, comisiones, payouts inversionista)
create table if not exists public.movimientos (
  id uuid primary key default gen_random_uuid(),
  opportunity_id bigint not null,
  investor_id uuid null,
  tipo text not null, -- cobro_prestatario | payout_inversionista | comision_plataforma
  amount numeric not null,
  currency text not null default 'BOB',
  ref_borrower_intent_id uuid null,
  ref_payout_id uuid null,
  nota text null,
  status text not null default 'pending', -- pending | posted | paid
  created_at timestamptz not null default now(),
  settled_at timestamptz null
);
-- Asegurar columnas nuevas si la tabla existía con esquema previo
...
-- Procesar pago de prestatario: marca intent pagado, genera payouts y movimientos
...
begin
  ...
  -- Asiento de cobro al prestatario (posted)
...
-- Marcar payout pagado y actualizar movimientos
...
-- Reabrir oportunidad sólo si no tiene pagos acreditados (evita ocultar oportunidades sin fondeo)
