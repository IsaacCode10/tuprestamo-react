-- Track notarization cost absorption in opportunities and keep RPC atomic updates aligned.

alter table if exists public.oportunidades
  add column if not exists originacion_bruta numeric,
  add column if not exists costo_notariado numeric,
  add column if not exists originacion_neta numeric,
  add column if not exists notariado_absorbido boolean default false,
  add column if not exists notariado_pagado_at timestamp with time zone;
