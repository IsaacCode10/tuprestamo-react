...
import './Modal.css'; // Asumimos que crearemos un archivo Modal.css para los estilos del modal
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { error } = await supabase.from('solicitudes').insert({
        ...
        saldo_deuda_tc: parseFloat(formData.saldo_deuda_tc),
        tasa_interes_tc: parseFloat(formData.tasa_interes_tc),
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content">
        ...
        <h2>Solicitud para {role === 'prestatario' ? 'Prestatarios' : 'Inversionistas'}</h2>
        <form onSubmit={handleSubmit}>
          ...
          {role === 'inversionista' && (
            <>
              {/* Aquí irían los campos específicos para inversionistas si fueran diferentes */}
              {/* Por ahora, solo usaremos los campos comunes para ambos roles en este formulario */}
              <p>Este es el formulario para inversionistas. Por ahora, solo se piden los datos comunes.</p>
            </>
          )}
